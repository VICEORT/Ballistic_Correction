...
VIDEO_PATH = "12月19日 中科大 东南.mp4"  # 新的视频源路径
...
while True:
    ...
    # 绘制轨迹（当前轨迹与上一次已完成轨迹）并标注消失点
    # 上一次已完成轨迹（灰色）
    if prev_trajectory and len(prev_trajectory) > 1:
        for i in range(1, len(prev_trajectory)):
            x1, y1, _ = prev_trajectory[i - 1]
            x2, y2, _ = prev_trajectory[i]
            cv2.line(frame, (int(x1), int(y1)), (int(x2), int(y2)), (160, 160, 160), 2)
        if last_disappearance_point is not None:
            dx, dy = last_disappearance_point
            cv2.circle(frame, (int(dx), int(dy)), 6, (0, 0, 255), -1)
            cv2.putText(frame, "Disappear", (int(dx) + 8, int(dy) - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)

    # 当前进行中的轨迹（黄色 -> green tail）
    if len(ball_trajectory) > 1:
        for i in range(1, len(ball_trajectory)):
            x1, y1, _ = ball_trajectory[i - 1]
            x2, y2, _ = ball_trajectory[i]
            cv2.line(frame, (int(x1), int(y1)), (int(x2), int(y2)), (0, 255, 255), 2)
        lx, ly, _ = ball_trajectory[-1]
        cv2.circle(frame, (int(lx), int(ly)), 4, (0, 255, 0), -1)
